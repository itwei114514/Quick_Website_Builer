...
.frame-vintage img {
  border: 5px solid #d4a574 !important;
  box-shadow: 0 3px 15px rgba(0,0,0,0.2), inset 0 0 0 1px #b8845a !important;
  filter: sepia(0.15) !important;
  border-radius: 2px !important;
}
.frame-neon img {
  border: 2px solid #ffffff !important;
  box-shadow: 0 0 8px #0ff, 0 0 15px #0ff, 0 0 30px rgba(0,255,255,0.4) !important;
  border-radius: 3px !important;
}
.frame-soft img {
...
class Element:
    ...
    object_fit: str = "contain"   # contain / cover / fill - 图片适应方式
    frame_style: str = "none"     # none / simple / float / rounded / vintage / neon / soft - 相框样式
    ...
    def from_dict(cls, d: dict) -> "Element":
        return cls(
            ...
            object_fit=d.get("objectFit", "contain"),
            frame_style=d.get("frameStyle", "none"),
        )
...
def _try_extract_video_thumb(video_path: str) -> Optional[str]:
    """提取视频文件的中间帧作为缩略图，返回临时 PNG 路径；失败则返回 None
    优先使用 OpenCV (cv2)，其次尝试 ffmpeg。
    """
    thumb_dir = os.path.join(tempfile.gettempdir(), "wsweb_thumbs")
    os.makedirs(thumb_dir, exist_ok=True)
    thumb_path = os.path.join(thumb_dir, f"thumb_{uuid.uuid4().hex}.png")
...
def element_to_html(elem: Element, media_mode: str = "embed") -> str:
    """将单个元素转换为HTML
    media_mode: "embed" → 本地文件 base64 嵌入（自包含HTML）
                "link"  → 保留文件路径（项目文件夹导出，文件已复制到输出目录）
    """
    style = (
        f"position:absolute;"
    ...
    if elem.type == "text":
        ...
        object_fit = elem.object_fit if hasattr(elem, 'object_fit') else "contain"
        frame_cls = f" frame-{elem.frame_style}" if (hasattr(elem, 'frame_style') and elem.frame_style != "none") else ""
        inner_html = f'<div class="{tag_class}{frame_cls}" style="{style}overflow:hidden;"><img src="{img_src}" style="width:100%;height:100%;object-fit:{object_fit};{br}"></div>'
        ...
        if os.path.isfile(video_src):
            if media_mode == "link":
                # 项目文件夹导出：文件已复制到输出目录，保留相对路径
                pass
            else:
                # 自包含模式：base64 嵌入（限制 15MB 避免 HTML 过大）
                # 自包含模式：base64 嵌入（限制 15MB 避免 HTML 过大）
                try:
                    file_size = os.path.getsize(video_src)
                    if file_size < 15 * 1024 * 1024:
                        with open(video_src, "rb") as f:
                            ext = os.path.splitext(video_src)[1].lower().replace(".", "")
                            VIDEO_EXTS = {"mp4": "mp4", "webm": "webm", "ogg": "ogg",
                                          "mov": "quicktime", "avi": "x-msvideo",
                                          "wmv": "x-ms-wmv", "flv": "x-flv"}
                            safe_ext = VIDEO_EXTS.get(ext)
                            if safe_ext:
                                b64 = base64.b64encode(f.read()).decode("utf-8")
                                video_src = f"data:video/{safe_ext};base64,{b64}"
                except Exception:
                    pass
        else:
...
def _build_page_html(page: Page, page_index: int, total_pages: int, nav_style: str = "floating",
                     media_mode: str = "embed") -> tuple:
    """生成单个页面的 HTML"""
    body_classes = []
    bg_style = ""
    s = page.settings
    ...
    for el in sorted(page.elements, key=lambda e: e.z_index):
        h = element_to_html(el, media_mode=media_mode)
        if h:
            elem_htmls.append("  " + h)
...
def generate_html(pages: list[Page], global_title: str = "", nav_style: str = "floating") -> str:
    ...
    has_frame = any(hasattr(el, 'frame_style') and el.frame_style != "none" for el in all_elements)
    if has_frame:
        css_parts.append(FRAME_STYLES_CSS)
...
def _build_page_html_static(page: Page, page_index: int, total_pages: int, elements: list,
                             nav_style: str = "floating", media_mode: str = "embed") -> str:
...
def generate_project_folder(pages: list[Page], folder_path: str, title: str = "", nav_style: str = "floating") -> str:
    ...
    for page in pages_copy:
        for el in page.elements:
            if el.type == "video" and os.path.isfile(el.content):
                os.makedirs(media_dir, exist_ok=True)
                fname = os.path.basename(el.content)
                dest = os.path.join(media_dir, fname)
                try:
                    shutil.copy2(el.content, dest)
                    el.content = f"media/{fname}"
                    has_video_media = True
                except Exception:
                    pass  # 复制失败则退回到 base64 嵌入

    # ---- index.html ----
    page_htmls = []
    media_mode = "link" if has_video_media else "embed"
    for i, page in enumerate(pages_copy):
        ph = _build_page_html_static(page, i, len(pages_copy), page.elements, nav_style, media_mode=media_mode)
        page_htmls.append(ph)
  ...
  <button class="nav-prev" disabled>◀</button>
  <span class="page-indicator">1 / {len(pages_copy)}</span>
  <div class="page-dots">{dots_html}</div>
  <button class="nav-next"{' disabled' if len(pages_copy) <= 1 else ''}>▶</button>
</nav>"""
    ...
    def _draw_element(self, elem: Element):
        ...
        if elem.type == "text":
            ...
            if elem.content and HAS_PIL:
                try:
                    if os.path.isfile(elem.content):
                        ...
                        # 统一转换到 RGB 确保兼容性（RGBA 在某些 tk 版本不渲染）
            ...
            if not thumb_shown:
                ...
                extra = " [链接]" if has_link else ""
                # 绘制白色底再写文字，确保文字始终可见
                self.create_rectangle(cx + 4, cy + ch // 2 - 18,
                                      cx + cw - 4, cy + ch // 2 + 18,
                                      fill="#ffffff", outline="", tags=tags)
                label_text = f"[图片] {img_label}{extra}"
                if elem.content and not os.path.isfile(elem.content):
                    label_text = f"[网络图片] {img_label}{extra}"
                self.create_text(cx + cw // 2, cy + ch // 2,
                                 text=label_text,
                                 font=("Microsoft YaHei", max(10, cw//20)),
                                 fill="#333333", anchor="center", tags=tags)
            # 相框样式指示
            if hasattr(elem, 'frame_style') and elem.frame_style != "none":
                frame_label = FRAME_NAMES.get(elem.frame_style, elem.frame_style)
                self.create_rectangle(cx + cw - 80, cy + ch - 18, cx + cw - 2, cy + ch - 2,
                                      fill="#0066cc", outline="", tags=("frame_bg",) + tags)
                self.create_text(cx + cw - 41, cy + ch - 10,
                                 text=frame_label,
                                 font=("Microsoft YaHei", 7), fill="white",
                                 anchor="center", tags=("frame_text",) + tags)
            # 图片适应方式指示
            if hasattr(elem, 'object_fit') and elem.object_fit != "contain":
                fit_names = {"cover": "截取", "fill": "拉伸"}
                fit_label = fit_names.get(elem.object_fit, "")
                self.create_text(cx + 4, cy + 4,
                                 text=f"📐{fit_label}",
                                 font=("Microsoft YaHei", 7), fill="#cc6600",
                                 anchor="nw", tags=("fit_label",) + tags)

        elif elem.type == "video":
            # 视频区域背景
            self.create_rectangle(cx, cy, cx + cw, cy + ch,
                                  outline=bc, width=bw, fill="#2a2a2a", tags=tags)
            vlabel = os.path.basename(elem.content) if elem.content else "视频"
            extra = " [链接]" if has_link else ""

            if elem.content:
                # 尝试用 OpenCV / ffmpeg 提取视频中间帧作为缩略图
                thumb_shown = False
                if HAS_PIL and os.path.isfile(elem.content) and cw > 40 and ch > 40:
                    try:
                        # 缓存键：文件路径 + 修改时间
                        mtime = os.path.getmtime(elem.content)
                        cache_key = f"vid_{elem.elem_id}_{mtime}"
                        if cache_key not in self._thumb_cache:
                            thumb_path = _try_extract_video_thumb(elem.content)
                        ...
                        if cache_key in self._thumb_cache:
                            self.create_image(cx + cw // 2, cy + ch // 2,
                                              image=self._thumb_cache[cache_key],
                ...
                if not thumb_shown:
                    # 没有缩略图 → 白色标签提示
                    fs = max(12, min(cw, ch) // 10)
                    self.create_rectangle(cx + cw//4, cy + ch//3,
                                          cx + cw*3//4, cy + ch*2//3,
                                          fill="#444444", outline="", tags=tags)
                    self.create_text(cx + cw // 2, cy + ch // 2 - fs//2,
                                     text=vlabel,
                                     font=("Microsoft YaHei", fs),
                                     fill="white", anchor="center", tags=tags)
                ...
                self.create_oval(px - ps, py - ps, px + ps, py + ps,
                                 fill="", outline="white", width=2,
                                 tags=tags)
                tri_size = ps * 0.35
                self.create_polygon(
                    px + tri_size * 0.4, py - tri_size,
                    px + tri_size * 0.4, py + tri_size,
                    px + tri_size * 1.2, py,
                    fill="white", outline="", tags=tags)
                # 左下角标签
                self.create_text(cx + 6, cy + ch - 8,
                                 text="[视频]",
                                 font=("Microsoft YaHei", max(7, ch//15)),
                                 fill="white", anchor="sw", tags=tags)
            else:
                fs = max(12, min(cw, ch) // 8)
                self.create_rectangle(cx + cw//6, cy + ch//3,
                                      cx + cw*5//6, cy + ch*2//3,
                                      fill="#444444", outline="", tags=tags)
                self.create_text(cx + cw // 2, cy + ch // 2 - fs//2,
                                 text=vlabel + extra,
                                 font=("Microsoft YaHei", fs), fill="white",
                                 anchor="center", tags=tags)
                self.create_text(cx + cw // 2, cy + ch // 2 + fs,
                                 text="点击选择视频文件",
                                 font=("Microsoft YaHei", max(8, fs-2)), fill="#cccccc",
                                 anchor="center", tags=tags)
...
class PropertyPanel(tk.Frame):
    ...
    def _add_image_controls(self, elem: Element):
        ...
        # ===== 图片适应方式 =====
        sep_fit = tk.Frame(f, height=1, bg="#dddddd")
        sep_fit.pack(fill="x", pady=6)
        tk.Label(f, text="📐 图片适应方式", font=("Microsoft YaHei", 10, "bold"),
                 bg="#f8f8f8", anchor="w").pack(fill="x", pady=(0, 4))

        object_fit_var = tk.StringVar(value=elem.object_fit)
        fit_hints = {
            "contain": "保持比例，空白处留白",
            "cover": "裁剪超出部分，填满整个区域",
            "fill": "拉伸变形，完全填满区域",
        }
        for key, label in OBJECT_FIT_NAMES.items():
            rb = tk.Radiobutton(
                f, text=label, variable=object_fit_var, value=key,
                bg="#f8f8f8", font=("Microsoft YaHei", 8),
                anchor="w", wraplength=200, justify="left",
                command=lambda: (
                    self._update_elem(object_fit=object_fit_var.get()),
                    fit_hint_label.config(text=fit_hints.get(object_fit_var.get(), ""))
                )
            )
            rb.pack(fill="x", pady=1)
        fit_hint_label = tk.Label(f, text=fit_hints.get(elem.object_fit, ""),
                                 font=("Microsoft YaHei", 7), bg="#f8f8f8", fg="#888888",
                                 wraplength=200, anchor="w")
        fit_hint_label.pack(fill="x", pady=(2, 6))

        # ===== 相框样式 =====
        sep_frame = tk.Frame(f, height=1, bg="#dddddd")
        sep_frame.pack(fill="x", pady=6)
        tk.Label(f, text="🖼 相框样式", font=("Microsoft YaHei", 10, "bold"),
                 bg="#f8f8f8", anchor="w").pack(fill="x", pady=(0, 4))

        frame_keys = list(FRAME_NAMES.keys())
        frame_labels = [FRAME_NAMES[k] for k in frame_keys]
        initial_frame = FRAME_NAMES.get(elem.frame_style, elem.frame_style)
        frame_var = tk.StringVar(value=initial_frame)
        frame_cb = ttk.Combobox(f, textvariable=frame_var, values=frame_labels,
                                state="readonly", width=22)
        frame_cb.pack(pady=(2, 2))

        frame_hints = {
            "none": "不添加相框",
            "simple": "白色边框 + 轻微阴影，经典照片边框",
            "float": "大幅阴影悬浮效果，让图片浮在页面上方",
            "rounded": "大圆角 + 柔和阴影，现代感设计",
            "vintage": "暖色木纹边框 + 复古色调，怀旧风格",
            "neon": "霓虹光晕边框，科技感十足",
            "soft": "宽大浅灰色边框 + 柔和阴影，优雅外观",
        }

        def on_frame_select(_event=None):
            idx = frame_cb.current()
            if idx >= 0:
                key = frame_keys[idx]
                self._update_elem(frame_style=key)
                frame_hint_label.config(text=frame_hints.get(key, ""))
            self.canvas.redraw()
        frame_cb.bind("<<ComboboxSelected>>", on_frame_select)

        frame_hint_label = tk.Label(f, text=frame_hints.get(elem.frame_style, ""),
                                   font=("Microsoft YaHei", 7), bg="#f8f8f8", fg="#888888",
                                   wraplength=200, anchor="w")
        frame_hint_label.pack(fill="x", pady=(2, 0))
